...
import { properties, type ListingStatus, type PropertyType } from "@/lib/properties";
...
export default function ListingBrowser({ status }: { status: ListingStatus }) {
  ...
  const neighborhoods = useMemo(
    () => Array.from(new Set(statusProperties.map((p) => p.neighborhood))).sort(),
    [statusProperties]
  );
  ...
  const filtered = useMemo(() => {
    ...
    const result = statusProperties.filter((p) => {
    ...
    return sorted;
  }, [statusProperties, selectedTypes, minPrice, maxPrice, bedrooms, neighborhood, sort]);
        ...
        <div>
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <h1 className="font-heading text-2xl font-bold text-navy-900">
              ...
              {status === "For Rent" ? "For Rent" : "Found"} in Mogadishu
            </h1>
